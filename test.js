...
const calculateColorants = (color, quantity, unit, shine) => {
  ...
  // Algorithm: Use RGB values to determine colorant intensity
  ...
  if (rgb.r > 50) {
    ...
    colorants["RL"] = (redRatio * 80).toFixed(2); // Red Lake
    if (rgb.r > 200) {
      colorants["RUL"] = (redRatio * 40).toFixed(2); // Red Umber Light
    }
  }
  ...
  if (rgb.g > 50 && rgb.r > 100) {
    ...
    colorants["YL"] = (yellowRatio * 70).toFixed(2); // Yellow
    if (rgb.g > rgb.b && rgb.r > rgb.b) {
      colorants["JL"] = (yellowRatio * 30).toFixed(2); // Yellow Oxide
    }
  }
  ...
  if (rgb.b > 50) {
    ...
    colorants["BL"] = (blueRatio * 75).toFixed(2); // Blue
    if (rgb.b > 150) {
      colorants["KXL"] = (blueRatio * 35).toFixed(2); // Blue Extra
    }
  }
  ...
  if (intensity < 0.3) {
    ...
    colorants["CL"] = (blackRatio * 90).toFixed(2); // Carbon Black
  } else if (intensity < 0.6) {
    const grayRatio = (0.6 - intensity) * baseMultiplier;
    colorants["IL"] = (grayRatio * 50).toFixed(2); // Iron Oxide
  }

  // Magenta/Purple adjustment
  if (rgb.r > 100 && rgb.b > 100 && rgb.g < 100) {
    const magentaRatio = ((rgb.r + rgb.b) / 510) * baseMultiplier;
    colorants["DL"] = (magentaRatio * 55).toFixed(2); // Deep Magenta
    colorants["VUL"] = (magentaRatio * 25).toFixed(2); // Violet
  }

  // Brown/Earth tones
  if (rgb.r > rgb.g && rgb.g > rgb.b && rgb.r < 200) {
    const brownRatio = ((rgb.r + rgb.g) / 510) * baseMultiplier;
    colorants["EL"] = (brownRatio * 45).toFixed(2); // Raw Umber
    colorants["TL"] = (brownRatio * 30).toFixed(2); // Raw Sienna
  }

  // Orange tones
  if (rgb.r > 200 && rgb.g > 100 && rgb.g < 200 && rgb.b < 50) {
    const orangeRatio = ((rgb.r + rgb.g) / 510) * baseMultiplier;
    colorants["LL"] = (orangeRatio * 65).toFixed(2); // Orange
    colorants["AXL"] = (orangeRatio * 35).toFixed(2); // Orange Extra
  }
  ...
  // Shine level adjustment - adds extra binder/medium
  ...
  // Apply shine multiplier to all colorants
  ...
  // Remove zero or very small values
  Object.keys(colorants).forEach((key) => {
...
export default function PantoneColorMixer() {
  ...
  const filteredColors = PANTONE_COLORS.filter((color) =>
      ...
      <div className="flex-1 max-w-7xl mx-auto p-6 w-full">
        <header className="mb-8">
          ...
          <p className="text-gray-600">
            Professional paint color matching system
          </p>
        </header>
          ...
          <div className="mb-8 bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <label className="text-sm text-gray-600">Compare up to:</label>
                <select
            ...
            <div className="flex h-32 rounded-lg overflow-hidden border border-gray-200">
              {comparisonColors.map((color, idx) => (
                ...
                >
                  <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all flex items-center justify-center">
                    <span className="text-white opacity-0 group-hover:opacity-100 text-sm font-medium px-2 py-1 bg-black bg-opacity-50 rounded">
                      Remove
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-6 gap-2 max-h-96 overflow-y-auto pr-2">
              {filteredColors.map((color) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="h-16 rounded-t relative"
                      style={{ backgroundColor: color.hex }}
                    >
                      {isInComparison && (
                    ...
                    <div className="p-1.5 bg-white rounded-b">
                      <p className="text-xs text-gray-800 truncate font-medium leading-tight">
            ...
            {selectedColor && (
              <div className="bg-white rounded-lg shadow-sm p-6">
                    ...
                    return (
                      <div className="space-y-2 pt-2 border-t border-gray-200">
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            <p className="text-gray-500 text-xs">HEX</p>
                            <p className="font-mono font-medium">
                              {selectedColor.hex}
                            </p>
                          </div>
                          <div>
                            <p className="text-gray-500 text-xs">RGB</p>
                            <p className="font-mono text-xs">
                              {props.rgb.r}, {props.rgb.g}, {props.rgb.b}
                            </p>
                          </div>
                          <div>
                            <p className="text-gray-500 text-xs">HSL</p>
                            <p className="font-mono text-xs">
                              {props.hsl.h}°, {props.hsl.s}%, {props.hsl.l}%
                            </p>
                          </div>
                          <div>
                            <p className="text-gray-500 text-xs">CMYK</p>
                            <p className="font-mono text-xs">
                          ...
                          <div>
                            <p className="text-gray-500 text-xs">Brightness</p>
                ...
                <button
                  ...
                  className="w-full mt-4 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm"
                >
                ...
                <div className="space-y-2">
                  ...
                  {Object.entries(
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-600">{amount} ml</span>
                    </div>
                  ))}
                </div>
              </div>
